...
// accounts / voting -- see sv_accounts.c. Separate from the existing
// `privileged` field above, which is an unrelated legacy id-Software
// mechanism (IP-address-based, gates kick/give in deathmatch -- see
// IsID() in sv_main.c). account_name sized 32 to match SV_ACCOUNT_NAME_LEN
// in sv_accounts.h (kept as a literal here to avoid pulling that header
// into this very-widely-included one).
	qboolean		authenticated;		// logged into an account this session
	qboolean		is_admin;		// only meaningful if authenticated
