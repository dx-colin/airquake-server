...
//				Sys_Printf ("clc_nop\n");
			...
			case clc_stringcmd:
				...
				if (Q_strncasecmp(s, "status", 6) == 0)
					...
					ret = 1;
				// AirQuake accounts/voting/admin patch (see sv_accounts.c) --
				...
				// need `cmd register ...` etc, not bare `register ...`).
					...
					ret = 1;
				else if (Q_strncasecmp(s, "logout", 6) == 0)
					ret = 1;
				else if (Q_strncasecmp(s, "vote", 4) == 0)
					ret = 1;
				else if (Q_strncasecmp(s, "admin", 5) == 0)
					ret = 1;
				if (ret == 2)
					Cbuf_InsertText (s);
				else if (ret == 1)
