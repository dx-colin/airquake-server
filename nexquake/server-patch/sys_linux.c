...
//	signal(SIGFPE, floating_point_exception_handler);
	...
	// -accountsdir <path>: see sv_accounts.c's SV_Accounts_SetDir doc
	// comment. Must run before Host_Init() -- SV_Accounts_Init() is called
	// from deep inside it (SV_Init) and reads the override at that point.
	j = COM_CheckParm ("-accountsdir");
	if (j && j < com_argc - 1)
		SV_Accounts_SetDir (com_argv[j + 1]);
...
//	parms.cachedir = cachedir;
	...
	// One-shot admin bootstrap/reset: `nqserver ... -createadmin <user> <pass>`.
	// Runs after Host_Init() so com_gamedir is resolved exactly like normal
	// play would use it, then exits before entering the game loop below.
	// Idempotent (updates the account if it already exists), so it doubles
	// as a password-reset path.
	j = COM_CheckParm ("-createadmin");
	if (j && j + 2 < com_argc)
	{
		qboolean ok = SV_Accounts_CreateAdmin (com_argv[j + 1], com_argv[j + 2]);
		return ok ? 0 : 1;
	}
...
/*
 * Stub functions for headless NetQuake server
 * Add to sys_linux.c for server builds
 */

void Sys_SendKeyEvents (void)
{
	// Stub function for headless server - no keyboard input needed
}
