...
ACCOUNTS / VOTING / ADMIN

Private (SV_ClientPrintf, one client only) vs public (SV_BroadcastPrintf,
everyone) feedback is chosen deliberately per command below -- credentials
and admin-denied messages are private, votes and rule changes are public.
See sv_accounts.c for the storage/hashing/attribution implementation.
...
register <username> <password>  -- creates an account and logs in
==================
*/
void Host_Register_f (void)
{
	char	errmsg[SV_ACCOUNT_ERR_LEN];

	if (cmd_source == src_command)
		return;	// server console/FIFO has no need for an account

	if (Cmd_Argc () < 3)
	{
		SV_ClientPrintf ("Usage: register <username> <password>\n");
		return;
	}

	if (SV_Accounts_Register (host_client, Cmd_Argv (1), Cmd_Argv (2), errmsg, sizeof (errmsg)))
		SV_ClientPrintf ("Registered and logged in as %s.\n", host_client->account_name);
	else
		SV_ClientPrintf ("Registration failed: %s\n", errmsg);
}
...
void Host_Login_f (void)
{
	...
	if (Cmd_Argc () < 3)
	{
		SV_ClientPrintf ("Usage: login <username> <password>\n");
		return;
	}

	if (host_client->authenticated)
	{
		SV_ClientPrintf ("Already logged in as %s -- logout first.\n", host_client->account_name);
		return;
	}

	if (SV_Accounts_Login (host_client, Cmd_Argv (1), Cmd_Argv (2), errmsg, sizeof (errmsg)))
		SV_ClientPrintf ("Logged in as %s%s.\n", host_client->account_name,
			host_client->is_admin ? " (admin)" : "");
	else
		SV_ClientPrintf ("Login failed: %s\n", errmsg);
}
...
void Host_Logout_f (void)
{
	...
	if (!host_client->authenticated)
	{
		SV_ClientPrintf ("You're not logged in.\n");
		return;
	}

	SV_ClientPrintf ("Logged out of %s.\n", host_client->account_name);
	SV_Accounts_Logout (host_client);
}
...
void Host_Vote_f (void)
{
	...
	if (Cmd_Argc () < 2)
	{
		SV_ClientPrintf ("Usage: vote <mapname>  (votable maps: %s)\n", sv_votable_maps.string);
		return;
	}

	SV_Accounts_CastVote (host_client, Cmd_Argv (1));
}
...
{
	"fraglimit", "timelimit", "hostname", "teamplay", "map", "changelevel", "kick", NULL
};

void Host_Admin_f (void)
{
	const char	*sub;
	char		cmdline[256];
	int		i, argc;
	...
	if (!host_client->authenticated || !host_client->is_admin)
	{
		SV_ClientPrintf ("Access denied: admin accounts only.\n");
		return;
	}

	if (Cmd_Argc () < 2)
	{
		SV_ClientPrintf ("Usage: admin <fraglimit|timelimit|hostname|teamplay|map|changelevel|kick> [args...]\n");
		return;
	}

	sub = Cmd_Argv (1);
	allowed = false;
	for (i = 0; sv_admin_allowed_cmds[i]; i++)
	{
		if (Q_strcasecmp ((char *)sub, (char *)sv_admin_allowed_cmds[i]) == 0)
		{
			allowed = true;
			break;
		}
	...
	if (!allowed)
	{
		SV_ClientPrintf ("Unknown or disallowed admin command: %s\n", sub);
		return;
	}
	...
	for (i = 1; i < argc; i++)
	{
		strncat (cmdline, Cmd_Argv (i), sizeof (cmdline) - strlen (cmdline) - 1);
		if (i < argc - 1)
	...
	// once-per-frame Cbuf_Execute() (before SV_RunClients()) is the safe
	// point for this to actually run.
...
void Host_InitCommands (void)
{
	...
	Cmd_AddCommand ("register", Host_Register_f);
	Cmd_AddCommand ("login", Host_Login_f);
