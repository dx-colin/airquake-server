...
cvar_t	sv_votable_maps = {"sv_votable_maps","",CVAR_NONE}; // space-separated map list; also mirrored in entrypoint.sh's AIRQUAKE_MAPS -- keep in sync
cvar_t	sv_vote_threshold = {"sv_vote_threshold","0.5",CVAR_NONE}; // fraction of eligible players needed to pass a map vote
...
void Host_Init (void)
{
	...
	Con_Printf ("\n========= Quake Initialized =========\n\n");
	// NexQuake's Nexus scans console output for its bundled WinQuake-derived
	// server's exact (unspaced) banner text to know when to query the
	// listening port/search path -- see monitorServerStartup in NexQuake's
	// nexus/internal/orch/console.go. QuakeSpasm's own banner above has
	// spaces added for readability and doesn't match, which otherwise makes
	// every server managed by Nexus time out as "failed to start" even
	// though it's actually running fine. Emit their exact string too so
	// Nexus's detection works without depending on our banner wording.
