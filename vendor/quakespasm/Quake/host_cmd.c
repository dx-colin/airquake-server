...
==================
*/
static void Host_Register_f (void)
{
...
==================
*/
static void Host_Logout_f (void)
{
	if (cmd_source == src_command)
		return;
...
static void Host_Admin_f (void)
{
	...
	{
		if (q_strcasecmp (sub, sv_admin_allowed_cmds[i]) == 0)
		{
			allowed = true;
			break;
		}
	...
	for (i = 1; i < argc; i++)
	{
		q_strlcat (cmdline, Cmd_Argv (i), sizeof (cmdline));
		if (i < argc - 1)
